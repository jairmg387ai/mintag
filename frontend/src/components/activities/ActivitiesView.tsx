...
function azureAuthModeLabel(mode?: string): string {
  if (mode === 'basic') return 'PAT básico'
  if (mode === 'bearer') return 'Bearer'
  return mode || 'desconocido'
}

function azureConfigSourceLabel(source?: string): string {
  if (source === 'local_db') return 'base de datos local'
  if (source === 'env') return 'variable de entorno'
  if (source === 'none' || !source) return 'sin configurar'
  return source
}
...
export function ActivitiesView() {
  ...
  async function handleUnapprove(id: number) {
    ...
    try {
      ...
      setRowErrors(prev => ({
        ...prev,
        [id]: e instanceof Error ? e.message : 'No se pudo quitar la aprobación',
      }))
    }
  }
  ...
  async function handleSaveAzureConfig() {
    ...
    try {
      ...
      setAzureConfig(cfg)
      setAzureToken('')
      setAzureConfigMessage('Token de Azure guardado. El token se oculta después de guardar.')
    } catch (e: unknown) {
  ...
  async function handleStartDeviceAuth() {
    ...
    try {
      ...
      setDeviceAuth(auth)
      setAzureConfigMessage('Inicio de sesión por dispositivo iniciado. Abre la URL de verificación, ingresa el código y completa el inicio de sesión aquí.')
    } catch (e: unknown) {
  ...
  async function handleCompleteDeviceAuth() {
    ...
    try {
      ...
      if (result.status === 'pending') {
        setAzureConfigMessage('La autorización sigue pendiente. Finaliza el inicio de sesión en el navegador e intenta completar de nuevo.')
        return
      }
    ...
    <div className="content-pad">
                        ...
                        {a.status === 'approved' && (
                          <>
                            <button
                              ...
                              aria-label="Quitar aprobación de la actividad"
                            >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ font: 'var(--text-caption)', color: 'var(--fg3)' }}>
            {approved.length} {approved.length === 1 ? 'entrada aprobada lista' : 'entradas aprobadas listas'} para subir
          </span>
        </div>
          ...
          >
            ...
            {uploadResult.failed_ids.length > 0 && (
              <div style={{ marginTop: 4 }}>
                IDs fallidos: {uploadResult.failed_ids.join(', ')} — {uploadResult.errors.join(', ')}
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            <span className={`chip ${azureConfig?.configured ? 'chip-done' : 'chip-pending'}`}>
              {azureConfig?.configured ? `Configurada (${azureAuthModeLabel(azureConfig.auth_mode)})` : 'No configurada'}
            </span>
            ...
            {azureConfig?.source && azureConfig.source !== 'none' && (
              <span style={{ font: 'var(--text-caption)', color: 'var(--fg3)' }}>Origen: {azureConfigSourceLabel(azureConfig.source)}</span>
            )}
            {azureConfig?.oauth_access_token_expires_at && (
              <span style={{ font: 'var(--text-caption)', color: 'var(--fg3)' }}>
                El acceso vence: {new Date(azureConfig.oauth_access_token_expires_at).toLocaleString()}
              </span>
            )}
          </div>
          <div style={{ padding: '12px', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <button className="btn btn-secondary btn-sm" onClick={handleStartDeviceAuth} disabled={deviceAuthLoading}>
                {deviceAuthLoading ? 'Procesando...' : 'Iniciar sesión por dispositivo'}
              </button>
              {deviceAuth && (
            ...
            <div style={{ font: 'var(--text-caption)', color: 'var(--fg3)' }}>
              Recomendado para Mintag. Guarda credenciales de renovación localmente y no muestra los tokens.
            </div>
            {deviceAuth && (
              <div style={{ padding: '10px 12px', background: 'var(--bg2)', borderRadius: 'var(--radius-md)', display: 'grid', gap: 6 }}>
                ...
                <div style={{ font: 'var(--text-caption)', color: 'var(--fg3)' }}>
                  Vence en {Math.round(deviceAuth.expires_in / 60)} minutos. Intervalo de consulta: {deviceAuth.interval}s.
                </div>
              </div>
            )}
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <span style={{ font: 'var(--text-caption)', color: 'var(--fg3)', width: '100%' }}>Alternativa con token manual</span>
            <select
              className="input"
              value={azureAuthMode}
          ...
          {azureAuthMode === 'bearer' && (
            <div style={{ font: 'var(--text-caption)', color: 'var(--amber-700)' }}>
              Los tokens de acceso Bearer vencen. Si Azure devuelve una respuesta de inicio de sesión, reemplaza este token por uno nuevo.
            </div>
          )}
          {azureConfigMessage && (
            <div style={{ font: 'var(--text-caption)', color: azureConfigMessage.includes('Failed') || azureConfigMessage.includes('No se pudo') ? 'var(--block-solid)' : 'var(--fg3)' }}>
              {azureConfigMessage}
            </div>
          )}
        </div>

...
export function TaskList({ tasks, onOpen }: TaskListProps) {
  return (
    <div className="card">
      <div
        style={{ display: 'grid', gridTemplateColumns: '28px 1fr 110px 120px 90px 90px 32px', gap: 12, padding: '8px 16px' }}
        className="task-list-header"
      >
        {['', 'Task', 'Status', 'Meeting', 'Owner', 'Priority', ''].map((h, i) => (
          <span key={i} style={{ fontSize: '0.72em', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--color-text3)' }}
            className={h === 'Meeting' ? 'col-meeting' : undefined}>
            {h}
          </span>
        ))}
      </div>
...
function TaskRow({ task: t, onClick }: { task: Task; onClick: () => void }) {
  return (
    <div
      className="row-clickable task-row-grid"
      onClick={onClick}
      style={{ display: 'grid', gridTemplateColumns: '28px 1fr 110px 120px 90px 90px 32px', alignItems: 'center', gap: 12, padding: '11px 16px' }}
    >
      ...
      <div>
        <button className="btn btn-ghost" onClick={e => { e.stopPropagation(); onClick() }} style={{ padding: '3px 6px' }}>

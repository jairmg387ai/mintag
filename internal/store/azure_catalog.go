...
// SyncAzureActivityLiveState persists the state, work item type, and
// assignee display name just fetched live from Azure (see
// handleGetAzureWorkItemStates) into the local catalog entry matching
// workItemID, so the portal shows a "last known" state on load instead of
// "—" every time until the next manual refresh, and so a work item that was
// reclassified in Azure (e.g. Bug -> Task) or reassigned picks up that
// change locally instead of staying stuck under stale data.
// workItemType is only applied when non-blank, so a caller that only has a
// state to report can't blank out a good stored type; assignedTo has no such
// guard and is always overwritten (including to blank for an unassigned work
// item), since "no longer assigned to anyone" is itself meaningful info, not
// a caller omission. assignedTo is UI-only (button-disable hint for
// Close/Recreate) — the actual enforcement in work_items.go's
// ensureAssignedToCaller always re-fetches the live assignee id, never reads
// this cached display name.
...
func (s *Store) SyncAzureActivityLiveState(ctx context.Context, workItemID int, state, workItemType, assignedTo string) error {
	...
	_, err := s.db.ExecContext(ctx,
		`UPDATE azure_activities
		 SET last_known_state = ?,
		     ...
		     last_known_assigned_to = ?
		 WHERE work_item_id = ?`,
		state, workItemType, workItemType, strings.TrimSpace(assignedTo), workItemID,
	)
	return err
}
